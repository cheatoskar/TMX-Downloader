# Submission notes — 1.7.0 (Chrome) / 2.7.0 (Firefox)
...
**`storage`** — Two values, both set by the user in the extension's own popup:
whether the replay bridge is enabled, and the pairing key the game mod
displays. Nothing else is stored, and neither value leaves the browser except
the key, which is sent only to the mod on 127.0.0.1 to authenticate the
extension to it.
...
TrackMania and opens a loopback-only socket, guarded by a random key it prints
in its own settings. The extension long-polls it for a replay the user has just
finished, fetches that file, and uploads it to the exchange. This is a
connection to a program on the user's own machine; no remote host is involved
and nothing is contacted while the bridge is off.
...
- it is off until switched on, and the key must be copied from the mod by hand
