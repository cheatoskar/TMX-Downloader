...
(function() {
   ...
   function createUI(dropdown) {
    ...
    async function handleDownload() {
        ...
        console.log('[TMX] 🚀 Starting download with URL:', apiUrl);
        ...
        if (shuffleTracks) {
            ...
            console.log('[TMX] 🔀 Tracks shuffled');
            ...
            console.log(`[TMX] 🎲 True random selection: ${tracks.length} from ${fullTracks.length} total`);
        }
        ...
        async function downloadTrack(track) {
            ...
            try {
                ...
                updateProgress((TMX_STATE.progress.current / TMX_STATE.progress.total) * 100, `Downloaded ${TMX_STATE.progress.current}/${TMX_STATE.progress.total} tracks...`);
            } catch (error) {
        ...
        alert(`✅ Download complete!\n${TMX_STATE.progress.current} of ${TMX_STATE.progress.total} tracks downloaded.`);
            ...
            if (createZip && TMX_STATE.abortController && TMX_STATE.abortController.signal.aborted && TMX_STATE.progress.current > 0) {
                ...
                try {
                    ...
                    a.click();
                    URL.revokeObjectURL(url);
                    console.log('[TMX] 📦 Partial ZIP downloaded');
